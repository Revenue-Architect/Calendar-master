...
Final corrective pass started at: `440b309acd6bdfd9fea1ce0b52c0b2ae0a8ed984`
Final implementation commit: `9361cc3c2938a7f3d0eb9d39f45dc18212c2c337`
...
fresh repository E2E run produced **323 passes and four failures** in the
existing timeline-chrome scroll cases (phone Day, phone Week, desktop Day, and
desktop Week). The same four tests fail in an exact focused rerun on the PR
base under the same Chromium binary, worker count, and command shape. They are
therefore documented as baseline failures, not attributed to v3; the full
suite is not described as an unqualified green run.
...
| Settled compositor hints | With the new rest-state selectors temporarily removed, body/source retained `will-change: transform, opacity, filter`; legacy cascade descendants retained `clip-path`. | V3 handoff hints were never released after the Composer reached the authoritative `open` stage, and a later legacy cascade rule overrode the earlier suppression. | Scoped `data-morph-stage="open"` and reduced-motion rules now force body/source/cascade descendants to `will-change: auto`; the override is ordered after the legacy cascade rule and leaves closing hints intact. | Computed-style assertions pass for settled open, OS reduced motion, and in-app reduced motion; no suppressed descendant retains `clip-path` will-change. |
| Delayed Composer handoff | With the `.28` body delay temporarily neutralized, the new shared-clock test failed because the body became visible before the contract boundary. | The body handoff must hold through 28% of the 350ms morph, not merely appear after a wall-clock sleep. | The production delay remains `calc(var(--nb-morph-dur) * .28)`; the test scrubs the real browser animations and asserts the 20%/25% hold, 40% entry, 200ms duration, and ≈98ms delay. | At 20%/25% body opacity is 0 with +36px translation, `.985` scale, and 1.5px blur; at 40% it is visibly entering. |
...
| `npx playwright test tests/e2e/motion.spec.js --workers=1 --grep "notch\|reduced"` | 26 passed |
| `npx playwright test tests/e2e/motion.spec.js --workers=1 --grep "ordinary Settings"` | 1 passed |
| `npx playwright test tests/e2e/motion.spec.js --workers=1` | 49 passed |
...
| Full `npx playwright test --workers=1` on final remediation tip | 323 passed, 4 failed |
| Focused `timeline-chrome-scroll.spec.js` on final tip | 0 passed, 4 failed |
| Same focused timeline suite on base `e3bca9f` | 0 passed, 4 failed — the same four failures |
| `npx playwright test tests/e2e/week-drag.spec.js --workers=1` on final tip | 8 passed |

The four residual failures are:
...
tests/e2e/timeline-chrome-scroll.spec.js
  ...
  phone: gives week view its hours back on the way down, and the heading back at midnight
...
The four failures reproduced on the untouched PR base under the same Chromium
binary, worker count, isolated port shape, and focused command. A separate
one-off Week drag failure appeared in an earlier full-suite run, but the
affected test passed in isolation on both revisions and the complete Week-drag
file passed 8/8 on the final tip; it was not reproducible and was not treated
as a v3 regression. No v3 remediation file is involved in the timeline-chrome
behavior, and no attempt was made to widen this PR into a timeline-chrome fix.
...
| 20% | Panel has moved left/down (`x≈707`, `y≈62`); source opacity is ≈.368 and has travelled ≈20px left. Body is still opacity 0 at +36px, `.985` scale, and 1.5px blur because 20% is before the 28% delay. | Panel has moved toward the bottom edge (`x≈−8.6`, `y≈378.8`); source opacity is ≈.368 and travelling left. Body remains opacity 0 at +36px, `.985` scale, and 1.5px blur. |
| 40% | Panel is progressing toward its destination (`x≈555`, `y≈133`); source opacity is ≈.083 at ≈−29px. Body is measurably entering at opacity ≈.508, +8.4px, scale ≈.9965, blur ≈.35px. | Panel is within about 4px of its destination; source is ≈.083 opaque at ≈−29px. Body is entering at the same shared handoff values (opacity ≈.508, +8.4px, ≈.9965 scale, ≈.35px blur). |
| 60% | Panel is near its final travel (`x≈453`, `y≈181`); source opacity is 0 at −32px. Body is ≈.997 opacity, within .14px of identity, and blur 0. | Panel is within ≈1px of its final bottom edge; source is gone and body is ≈.997 opacity at identity with no blur. |
| 80% | Panel is within ≈2px of final bounds (`x≈418`, `y≈197`); body is identity, readable, and the handoff is no longer visually active. | Panel is flush to the viewport edge (`x≈−.06`, `y≈386.8`); body is identity and no horizontal gap or white flash is visible. |
| 100% | After the wall-clock `open` stage cleanup, panel/body are identity, body opacity/filter are 1/none, source is opacity 0 with identity transform/filter, and all compositor hints are `auto`. | Same final contract; the full-width sheet is readable, source is hidden, and no residual transform/filter/will-change tail remains. |
...
- **Close:** a live desktop close sample reported `data-morph-stage="closing"`,
  body opacity ≈.008 with a positive ≈32px `translateX` to the right, source
  opacity ≈.992 returning from the left, and both using the 250ms close token.
  The panel and notch scrim moved as one surface; the underlying timeline
  returned without a stale scrim or snapped edge.
...
- **Performance:** the first-open path was inspected at desktop and mobile.
  ...
  paths.
- **Theme matrix:** dark/high-chroma `Obsidian / Acid`, light `Cream /
  Terracotta`, and a crimson high-chroma accent were opened in Chrome. The
  Composer retained readable contrast, intact controls, and the same bounded
  surface geometry in each; no theme-specific clip, overflow, or focus defect
  appeared.
- **Reduced motion:** Chrome OS `prefers-reduced-motion: reduce` was enabled
  during a live 390×601 open. The body was immediately readable with
  `animation:none`, identity transform, no filter, and `will-change:auto`; the
  source was hidden with the same compositor cleanup.
- **Navigation interlink:** at 390×601 the mobile drawer opened with the
  calendar return rail flush to the right edge, then returned to the calendar
  without a blank frame. Timeline/Actions/Settings and Event/Action inspector
  paths were also opened and closed in Chrome; all remained generic trigger
  Sheets and did not inherit notch choreography.
...
1. Track the four timeline-chrome scroll failures in their own regression PR;
